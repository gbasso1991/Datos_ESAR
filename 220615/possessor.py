...
#Obtengo frecuencias cuya intensidad relativa supera umbral dado por el filtro
    indices,_=find_peaks(abs(g),height=max(g)*filtro)
...
#bypass para utilizar un solo archivo de calibracion 22 Jun 22

una_sola_calibracion = 1 
...
concentracion = 7910 #[concentracion]= g/m^3 (1e4 g/m^3 == 10 g/l) (Default = 10000 g/m^3)
...
if (todos!=1) and (una_sola_calibracion!=1): 
    '''Selecciono 1 o + archivos de muestra, que viene acompañado de archivo de fondo y calibracion''' 
    ...
    #%%            
# =============================================================================
#     if una_sola_calibracion==1:
#         '''Selecciono 1 solo archivo de calibracion'''
#         root = tk.Tk()
#         root.withdraw()
#         texto_encabezado = "Seleccionar archivo de calibracion:"
#         path_c=filedialog.askopenfilenames(title=texto_encabezado,filetypes=(("Archivos .txt","*.txt"),("Archivos .dat","*.dat"),("Todos los archivos","*.*")))
#         directorio = path_c[0].rsplit('/',maxsplit=1)[0]
#                         
# =============================================================================
#%%
...
for k in range(len(fnames_m)):
    ...
    campo_ua0_c = delta_t_c*cumtrapz(fem_campo_c,initial=0) #[campo_ua0_c]=V*s
    ...
    magnetizacion_ua0_c = delta_t_c*cumtrapz(R_c,initial=0)#[magnetizacion_ua0_c]=V*s
    ...
    if Analisis_de_Fourier == 1:
        ...
        fig4_fourier.savefig(fnames_c[k][:-4]+'_Espectro_Fourier_C_{}_'.format(k)+str(fecha_nombre)+'.png',dpi=300,facecolor='w')
        
        fig5_fourier.savefig(fnames_c[k][:-4]+'_Rec_impar_cal_{}_'.format(k)+str(fecha_nombre)+'.png',dpi=300,facecolor='w')
        ...
        magnetizacion_ua0_c = delta_t_c*cumtrapz(R_c,initial=0)
    ...
    # Campo H(t)
    campo_ua0_m = delta_t_m*cumtrapz(fem_campo_m,initial=0) #V*s
    campo_ua_m = campo_ua0_m - np.mean(campo_ua0_m)
    campo_m = (campo_ua_m/max(campo_ua_m))*C_norm_campo #[campo_m]=A/m

    magnetizacion_ua0_m = delta_t_m*cumtrapz(R_m,initial=0)
    ...
    Area_ciclo = abs(trapz(magnetizacion_m_des,campo_m)) 
    ...
    Area_cal = abs(trapz(magnetizacion_c_des,campo_c)) #[Area_cal]=(A/m)^2
